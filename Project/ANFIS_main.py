...
from ANFISPredictor import Predictor, PredictorData
...
window = Tk()
window.title("Heart Disease Detector ANFIS")
...
def getdata() :
    try:
        ...
        if result < 0.50:
            value_print = "NOT LIKELY"
        elif 0.5 <= result < 0.75:
            value_print = "POSSIBLY"
        else:
            value_print = "MOST LIKELY"

        entry_result.config(state=NORMAL)
        entry_result.delete(0, END)
        entry_result.insert(END, str(value_print))
        entry_result.config(state=DISABLED)

        prediction_value.config(text=result, fg="black")
